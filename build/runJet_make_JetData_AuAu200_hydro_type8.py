...
MAINGENERATOR = "Gen_AuAu200_JetData"
wantDir = "hydro_files_AuAu200"
HYDRO_CONFIG_DIR = "hydro_files_AuAu200_config"
...
SHARED_HYDRO_DIR = Path("/alice/data/dongguk/hydro_files_AuAu200")
...
def build_run_script(
    ...
    if [[ -z "$HYDRO_SOURCE_DIR" ]]; then
      echo "ERROR: HYDRO_SOURCE_DIR is not set. Provide a shared hydro_files_AuAu200 path via the environment." >&2
      exit 1
    fi
...
def write_condor_submit(
    ...
    if not SHARED_HYDRO_DIR.exists():
        raise FileNotFoundError(f"hydro_files_AuAu200 directory not found at {SHARED_HYDRO_DIR}")
...
def ensure_hydro_symlink(base_path: Path) -> None:
    """Ensure hydro_files_AuAu200 points at the shared hydro directory."""
    if not SHARED_HYDRO_DIR.exists():
        raise FileNotFoundError(f"hydro_files_AuAu200 directory not found at {SHARED_HYDRO_DIR}")
